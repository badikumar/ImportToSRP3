        ...
        public override List<Individual> SerializeIndividuals()
        {
            try
            {
                ...
                foreach (DataRow row in Result.Tables[0].Rows)
                {
                    ...
                    //Comments
                    var comments = Convert.ToString(row[Result.Tables[0].Columns[17]]);
                    i.Comments = comments;

                    //Archived
                    var isArchived = FileHelpers.ConvertYesNoToBoolean(Convert.ToString(row[Result.Tables[0].Columns[18]]));
